...
class TestJwtUtilities:
    """Unit tests for JWT creation, validation, and expiration handling."""
...
class TestGoogleOAuthRoutes:
    ...
    def test_callback_token_exchange_network_error(
        ...
        with patch("httpx.AsyncClient.post", side_effect=mock_post):
            response = client.get("/auth/google/callback", params={"code": "test_code"})
    ...
    def test_callback_token_exchange_failure(
        ...
        with patch("httpx.AsyncClient.post", return_value=mock_resp):
            response = client.get(
                "/auth/google/callback", params={"code": "expired_code"}
            )
            assert response.status_code == status.HTTP_400_BAD_REQUEST
    ...
    def test_callback_userinfo_missing_email(
        ...
        with (
            ...
            response = client.get(
                "/auth/google/callback", params={"code": "valid_code"}
            )
            assert response.status_code == status.HTTP_400_BAD_REQUEST
            assert "did not provide an email" in response.json()["detail"]

    def test_callback_userinfo_unverified_email(
        self, client: TestClient, monkeypatch: pytest.MonkeyPatch
    ) -> None:
        monkeypatch.setattr(settings, "google_client_id", "test-client-id")
