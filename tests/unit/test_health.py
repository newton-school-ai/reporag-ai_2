...
Two endpoints with deliberately different jobs, tested separately:

* ``GET /health`` is the liveness probe. It must stay static, dependency-free
  and fast, because an orchestrator polls it every few seconds and will
  restart the process if it stops answering.
* ``GET /api/v1/health`` is the diagnostic view. It probes each dependency
  and reports them side by side, so an operator can tell which one is down.

The component probes are exercised directly rather than through HTTP, so
each failure mode is pinned without standing up a real Neo4j or Qdrant. No
...
class TestLivenessProbe:
    ...
    def test_does_not_probe_dependencies(
        ...
        # The whole point of a separate liveness endpoint: a slow Neo4j must
        # never make an orchestrator restart a healthy process.
...
class TestComponentHealth:
    ...
    def test_unconfigured_component_also_degrades(
        ...
        # Distinct from "error": nothing is broken, the deployment is
        # incomplete. Both are short of healthy.
    ...
    def test_degraded_still_returns_200(
        ...
        # A 503 would tell a load balancer to drop a node that can still
        # serve every endpoint not needing the dead component.
...
class TestNeo4jProbe:
    ...
    def test_unreachable_server_is_an_error_with_the_reason(
        ...
        # The reason is the point: "neo4j: error" alone sends an operator
        # looking through logs for what this already knows.
...
class TestLlmProbe:
    ...
    def test_placeholder_key_is_not_configured(
        ...
        # An unedited copy of .env.example must not report healthy; the
        # first real query would otherwise be what discovers the problem.
    ...
    def test_no_network_call_is_made(self, monkeypatch: pytest.MonkeyPatch) -> None:
        ...
        # A live completion on every poll would cost money and seconds; the
        # probe reports from configuration by design.
        import httpx
