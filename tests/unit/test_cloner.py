...
from src.reporag.ingestion.cloner import CloneError, FileInfo, RepoCloner
...
@pytest.mark.asyncio
async def test_async_clone_and_discovery_success(local_git_repo: Path) -> None:
    """Verify cloning from a local repository path succeeds and finds files."""
    cloner = RepoCloner()
    manifest = await cloner.clone_and_discover(str(local_git_repo))
...
@pytest.mark.asyncio
async def test_clone_failure_handling() -> None:
    """Verify cloning from a non-existent URL raises a CloneError."""
    cloner = RepoCloner()
    with pytest.raises(CloneError):
        await cloner.clone_and_discover(
...
@pytest.mark.asyncio
async def test_config_driven_extension_loading(monkeypatch: pytest.MonkeyPatch) -> None:
...
@pytest.mark.asyncio
async def test_supported_language_detection(local_git_repo: Path) -> None:
...
@pytest.mark.asyncio
async def test_custom_extension_overrides(local_git_repo: Path) -> None:
...
@pytest.mark.asyncio
async def test_ignored_directory_filtering(tmp_path: Path) -> None:
    ...
    cloner = RepoCloner()
    manifest = await cloner.clone_and_discover(str(repo_dir))
...
@pytest.mark.asyncio
async def test_manifest_metadata_validation(local_git_repo: Path) -> None:
    """Verify FileInfo attributes match the actual file properties."""
    cloner = RepoCloner()
    manifest = await cloner.clone_and_discover(str(local_git_repo))
    ...
    assert isinstance(py_file, FileInfo)
...
@pytest.mark.asyncio
async def test_local_repository_cloning(local_git_repo: Path) -> None:
...
@pytest.mark.asyncio
async def test_file_uri_normalization(
    local_git_repo: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    """Verify that local paths are converted into file:// URIs."""
    cloner = RepoCloner()

    original_execute = cloner._execute_async_clone
    called_repo_url = ""

    async def mock_execute(
        repo_url: str, target_dir: Path, branch: str | None, depth: int
    ) -> None:
        nonlocal called_repo_url
        called_repo_url = repo_url
        await original_execute(repo_url, target_dir, branch, depth)

    monkeypatch.setattr(cloner, "_execute_async_clone", mock_execute)

    await cloner.clone_and_discover(str(local_git_repo))
    assert called_repo_url.startswith("file://")
    cloner.cleanup()


@pytest.mark.asyncio
async def test_configured_clone_depth_usage(
    local_git_repo: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    """Verify cloner respects clone_depth configuration from Settings."""
    cloner = RepoCloner()
    ...
    await cloner.clone_and_discover(str(local_git_repo))
    assert called_depth == 5
    cloner.cleanup()


@pytest.mark.asyncio
async def test_explicit_depth_override_behavior(
    local_git_repo: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    """Verify runtime depth argument overrides Settings clone_depth."""
    cloner = RepoCloner()
    cloner.default_depth = 1
    ...
    await cloner.clone_and_discover(str(local_git_repo), depth=3)
    assert called_depth == 3
    cloner.cleanup()


@pytest.mark.asyncio
async def test_branch_selection(tmp_path: Path) -> None:
    ...
    manifest = await cloner.clone_and_discover(str(repo_dir), branch="feature-xyz")
...
@pytest.mark.asyncio
async def test_manifest_ordering_guarantees(tmp_path: Path) -> None:
...
@pytest.mark.asyncio
async def test_cleanup_verification(local_git_repo: Path) -> None:
...
@pytest.mark.asyncio
async def test_repository_size_limit_enforcement(local_git_repo: Path) -> None:
...
@pytest.mark.asyncio
async def test_shallow_clone_depth_validation(tmp_path: Path) -> None:
    ...
    cloner = RepoCloner()
    # Clone with depth=1
    await cloner.clone_and_discover(str(repo_dir), depth=1)
...
@pytest.mark.asyncio
async def test_clone_and_discover_error_propagation(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    """Verify non-CloneError exceptions are wrapped in CloneError."""
    cloner = RepoCloner()

    async def mock_execute(*args, **kwargs) -> None:
        raise ValueError("Simulated unexpected failure")

    monkeypatch.setattr(cloner, "_execute_async_clone", mock_execute)

    with pytest.raises(CloneError) as exc_info:
        await cloner.clone_and_discover("https://github.com/some/repo")
...
async def test_cleanup_on_error(
    ...
    def mock_discover(base_path: Path) -> list[FileInfo]:
...
@pytest.mark.asyncio
async def test_default_auto_detect_branch(
    local_git_repo: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
