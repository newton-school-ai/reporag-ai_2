...
* Blocking work goes to the threadpool: Every pipeline component is
  ...
  event loop for the whole LLM round trip. The route awaits run_in_threadpool.
...
class QueryRequest(BaseModel):
    ...
    repo_id: int | None = Field(
        default=None,
        validation_alias=AliasChoices("repo_id", "repository_id"),
        ge=1,
...
class _RetrievalEngineAdapter:
    ...
    SubQueryExecutor expects search_* methods over free text. This bridges
    the differences across BM25, vector search, and graph traversal.
    """
...
_CODE_SHAPED_RE = re.compile(r"[._]|[a-z][A-Z]")
...
def _extract_symbols(query: str, *, limit: int = 5) -> list[str]:
    ...
    for match in _IDENTIFIER_RE.finditer(query):
        token = match.group(0)
        if token in seen or not _CODE_SHAPED_RE.search(token):
            continue
        seen.add(token)
        found.append(token)
    return found[:limit]
...
def _run_pipeline(
    ...
    if top_k != settings.rerank_top_k and isinstance(executor, SubQueryExecutor):
        # Honour a per-request top_k without mutating the shared executor.
        executor = SubQueryExecutor(
            executor.engine, router=getattr(executor, "router", None), top_k=top_k
        )
    ...
    facts = {
        ...
        "strategies_used": strategies_list,
        "sources": sorted({r.file_path for r in results}),
        "result_count": len(results),
        "needs_decomposition": getattr(
            plan, "needs_decomposition", len(plan.steps) > 1
        ),
...
async def query(
    ...
    return QueryResponse(
        ...
        metadata=QueryMetadata(
            ...
            strategies_used=facts["strategies_used"],
            ...
            prompt_tokens=getattr(prompt, "token_count", 0),
            truncated=getattr(prompt, "truncated", False),
            needs_decomposition=facts["needs_decomposition"],
            num_sub_queries=facts["num_sub_queries"],
            success=True,
        ),
    )
