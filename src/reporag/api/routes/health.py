...
async def _check_database(session: AsyncSession) -> ComponentHealth:
    ...
    # Report the dialect rather than the URL, which carries a password.
...
def _check_neo4j() -> ComponentHealth:
    ...
    Deliberately uses the driver directly rather than
    :func:`~reporag.graph.neo4j_store.GraphStore`, whose constructor retries
    three times with exponential backoff -- roughly six seconds of sleeping
    that a health check must not pay for.
    """
...
def _check_llm() -> ComponentHealth:
    ...
    An unedited ``.env`` copied from ``.env.example`` still contains
    placeholder keys such as ``sk-your-key-here``. Those are treated as
    unset -- reporting them healthy would mean the first real query, not
    this endpoint, is what discovers the key was never filled in.
    """
    # Reuses the config module's placeholder set so the two can never
    # disagree about what counts as a configured secret.
...
async def health(
    ...
    """
    # The Neo4j and Qdrant probes are blocking socket calls, so they run in
    # the threadpool rather than on the event loop, and concurrently with
    # each other -- the endpoint costs one timeout, not the sum of them.
